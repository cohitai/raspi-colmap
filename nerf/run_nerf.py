#! /home/liteandfog/.pyenv/versions/tensorflow2/bin/python
    ...
    with writer.as_default():
        ...
        for i in range(start, N_iters):
            ...
            if i % args.i_print == 0 or i < 10:
                ...
                if global_step.numpy() % args.i_print == 0:
                    tf.summary.scalar('loss', loss,step=global_step.numpy())
                    tf.summary.scalar('psnr', psnr,step=global_step.numpy())
                    tf.summary.histogram('tran', trans,step=global_step.numpy())
                    if args.N_importance > 0:
                        tf.summary.scalar('psnr0', psnr0,step=global_step.numpy())
                ...
                if i % args.i_img == 0:
                    ...
                    if global_step.numpy() % args.i_print == 0:
                        tf.summary.image('rgb', to8b(rgb)[tf.newaxis],step=global_step.numpy())
                        tf.summary.image(
                            'disp', disp[tf.newaxis, ..., tf.newaxis],step=global_step.numpy())
                        tf.summary.image(
                            'acc', acc[tf.newaxis, ..., tf.newaxis],step=global_step.numpy())

                        tf.summary.scalar('psnr_holdout', psnr,step=global_step.numpy())
                        tf.summary.image('rgb_holdout', target[tf.newaxis],step=global_step.numpy())
                    ...
                    if args.N_importance > 0:
                        ...
                        if global_step.numpy() % args.i_print == 0:
                            tf.summary.image(
                                'rgb0', to8b(extras['rgb0'])[tf.newaxis],step=global_step.numpy())
                            tf.summary.image(
                                'disp0', extras['disp0'][tf.newaxis, ..., tf.newaxis],step=global_step.numpy())
                            tf.summary.image(
                                'z_std', extras['z_std'][tf.newaxis, ..., tf.newaxis],step=global_step.numpy())
